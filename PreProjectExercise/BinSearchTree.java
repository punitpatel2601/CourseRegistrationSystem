...
/**
 * The following class called BinSearchTree creates an empty tree, and allows
 * the user of the class to insert new nodes into the tree.
 */
...
class BinSearchTree {
	...
	public void insert(String id, String faculty, String major, String year) {
		...
		if (root == null)
			...
			// the following if...else block attaches the new node to the left or right
			// wing.
			...
			// if data already exists, send an error message to the user.
				...
				JOptionPane.showMessageDialog(null, "\nCannot insert: data already" + " exists: \n" + id, " Warning",
	...
	/**
	 * The following method finds and returns a reference to a node with the target
	 * id or returns null, if it fails to find a node containing target id.
	 * 
	 * @param start     - starting point of the tree (root node)
	 * @param target_id - id number that is being searched for
	 * @return - node that matches its id with the target id. Otherwise returns
	 *         null.
	 */
	public Node find(Node start, String target_id) {
		if (start == null)
			return null;
		if (start.data.id.equals(target_id))
			return start;
	...
	/**
	 * The following function returns a reference to the node that the new node must
	 * be attached to. Or returns null if such a node does not exist.
	 * 
